...
//		}
	...
	public void showLink(Book document, Key key) {
        ...
        if (!linksWindowWasVisible) {
        	linksWindow.getWindowLayout().setState(WindowLayout.WindowState.SPLIT);
	...
	public void maximiseWindow(Window window) {
		...
		if (!window.isLinksWindow()) {
			windowRepository.getDedicatedLinksWindow().getWindowLayout().setState(WindowLayout.WindowState.CLOSED);
		}
