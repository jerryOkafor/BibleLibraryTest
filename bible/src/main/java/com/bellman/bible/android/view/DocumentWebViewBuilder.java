...
import com.bellman.bible.android.control.window.Separator;
...
public class DocumentWebViewBuilder {
	...
	public DocumentWebViewBuilder(Activity mainActivity) {
        ...
        Resources res = Bible.getApplication().getResources();
...
//	}
	...
	private View createDefaultWindowActionButton(Window window) {
		...
		if (window.getDefaultOperation().equals(Window.WindowOperation.CLOSE)) {
		    // close button for the links window
		    defaultWindowActionButton = createCloseButton(window);
		} else if (window.getDefaultOperation().equals(Window.WindowOperation.MAXIMISE)) {
